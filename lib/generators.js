...
function generateModel(path, modelName, modelFields, generateMethod, ts, cb) {
    ...
    model = model.replace(/{modelName}/, formatTools.capitalizeFirstLetter(modelName));
...
function generateRouter(path, modelName, generateMethod, ts, cb) {
    ...
    if (generateMethod === 't') {
        ...
        ft.createDirIfIsNotDefined(path, modelName, function () {
            router = router.replace(/{controllerPath}/g, '\'./' + modelName + '.controllers.' + extension + '\'');
...
function generateController(path, modelName, modelFields, generateMethod, ts, cb) {
    ...
    controller = controller.replace(/{modelName}/g, formatTools.capitalizeFirstLetter(modelName));
    ...
    if (generateMethod === 't') {
        ...
        ft.createDirIfIsNotDefined(path, modelName, function () {
            ...
            ft.writeFile(path + '/' + modelName + '/' + modelName + '.controllers.' + extension, controller, null, cb);
        });
    }
}
